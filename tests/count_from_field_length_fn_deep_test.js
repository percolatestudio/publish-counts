if (Meteor.isServer) {
  ...
  Meteor.methods({
    setup_deep_countFromFieldLength_fn: function (testId) {
      H.insert(testId, 0, {a: {b: [1, 2, 3]}});
      H.insert(testId, 1, {a: {b: [1, 2, 3, 4]}});
    },
    addDoc_deep_countFromFieldLength_fn: function (testId) {
      H.insert(testId, 2, {a: {b: [1, 2]}});
    },
    updateDoc_deep_countFromFieldLength_fn: function (testId) {
      // add 2 elements to array of doc 0
      H.update(testId, 0, {$set: {'a.b': [1, 2, 3, 4, 5]}});
    },
    removeDoc_deep_countFromFieldLength_fn: function (testId) {
      H.remove(testId, 0);
    },
    addField_deep_countFromFieldLength_fn: function (testId) {
      H.update(testId, 0, {a: {b: [1, 2, 3, 4, 5]}});
    },
    removeField_deep_countFromFieldLength_fn: function (testId) {
      H.update(testId, 0, {});
    },
  });
}
